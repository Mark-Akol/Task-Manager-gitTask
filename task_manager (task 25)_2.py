...
# In the event that the user inputs 'gr', then reports will be generated with statistics
# about the tasks and each user. The logged in user must be "admin"
# to access this function.
def generate_reports():

    # Be sure to check if the user that is logged in is "admin"
    if username == "admin":
        ...
        # Create the neccessary variables that will be used.
        ...
        ###### task_overview.txt ######
        # Based on each the information in each task, loop through and update the necessary variables
        for x in range(0, len(tasks)):
            if tasks[1] == "Yes":
                num_completed += 1
            elif tasks[-1] == "No":
                ...
                # Make sure to convert the Due Date to a a comparable datetime object
                # of the existing current date to suite the due date.
                task_date = datetime.strptime(tasks[x][4], '%Y-%m-%d')
                if datetime.date(datetime.now()) < task_date.date():
                    num_inc_overdue += 1

            # If the number of tasks is equal to zero, to limit the
            # calculation below from dividing by zero, we would rather instead make the
        ...
        # Apply the function that allows the statistics calculated above to be written to the file.
        ...
        num_users = len(users)
        user_over_w.write("User Overview - The statistics pertaining to all registed users in task_manager.py\n\n")
        ...
        for x in range(0, len(users)):
            ...
            for y in range(0, len(tasks)):
                ...
                # Make a check , to observe if the current user being looped has the task asssigned to them
                # Using nfo from each task, update the variables(similar to above)
            ...
            # Write the info to the file from the statistics calculations above.
    ...
    # If the current user does not have the authority to use the function
    # Alert the user of their limited accesibility
    else:
